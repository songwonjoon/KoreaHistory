...
<table class="photo_view_table">
		...
		<tr>
			...
			<c:if test="${photo_view.pfile ne null }">
			<td>
				<a href="pdownload?pp=../resources/upload/&p=
				${photo_view.pfile }
				&pid=${photo_view.pid }"><img alt="${photo_view.pfile }" 
				src="../resources/upload/${photo_view.pfile }" style="width: 900px; height: 500px;"></a>
			</td>
			</c:if>
			<c:if test="${photo_view.pfile eq null }">
		...
		<tr>
			...
			<td class="photo_view_td_c">
			${photo_view.p_content }</td>
		</tr>
		<tr>
			<td class="photo_view_asub" colspan="2">
			<c:choose>
				<c:when test="${id eq photo_view.pid }">
							<a href="photo_modify?p_num=${photo_view.p_num }">
				...
				<c:when test="${id eq 'MASTER' }"> 
							...
							<a class="photo_view_return" href="photo_list">목록보기</a>
			</td>
		</tr>
</table>
</body>
