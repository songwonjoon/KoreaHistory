...
<table class="photo_view_table">
		...
		<tr>
			...
			<c:if test="${photo_view2.pfile ne null }">
			<td>
				<a href="pdownload2?pp=../resources/upload/&p=
				${photo_view2.pfile }
				&pid=${photo_view2.pid }"><img alt="${photo_view2.pfile }" 
				src="../resources/upload/${photo_view2.pfile }" style="width: 900px; height: 500px;"></a>
			</td>
			</c:if>
			<c:if test="${photo_view2.pfile eq null }">
		...
		<tr>
			...
			<td class="photo_view_td_c">
			${photo_view2.p_content }</td>
		</tr>
		<tr>
			<td class="photo_view_asub" colspan="2">
			<c:choose>
				<c:when test="${id eq photo_view2.pid }">
							<a href="photo_modify2?p_num=${photo_view2.p_num }">
				...
				<c:when test="${id eq 'MASTER' }"> 
							...
							<a class="photo_view_return" href="photo_list2">목록보기</a>
			</td>
		</tr>
</table>
</body>
