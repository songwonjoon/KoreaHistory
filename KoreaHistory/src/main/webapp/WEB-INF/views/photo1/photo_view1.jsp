...
<table class="photo_view_table">
		...
		<tr>
			...
			<c:if test="${photo_view1.pfile ne null }">
			<td>
				<a href="pdownload1?pp=../resources/upload/&p=
				${photo_view1.pfile }
				&pid=${photo_view1.pid }"><img alt="${photo_view1.pfile }" 
				src="../resources/upload/${photo_view1.pfile }" style="width: 900px; height: 500px;"></a>
			</td>
			</c:if>
			<c:if test="${photo_view1.pfile eq null }">
		...
		<tr>
			...
			<td class="photo_view_td_c">
			${photo_view1.p_content }</td>
		</tr>
		<tr>
			<td class="photo_view_asub" colspan="2">
			<c:choose>
				<c:when test="${id eq photo_view1.pid }">
							<a href="photo_modify1?p_num=${photo_view1.p_num }">
				...
				<c:when test="${id eq 'MASTER' }"> 
							...
							<a class="photo_view_return" href="photo_list1">목록보기</a>
			</td>
		</tr>
</table>
</body>
